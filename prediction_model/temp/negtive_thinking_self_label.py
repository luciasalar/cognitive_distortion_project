...
# In[45]:
data = pd.read_csv('self_label_distortion2.csv')
...
selected = ['userid', 'marital_status', 'ethnicity', 'gender','age','relationship_status', 'network_size','negative_yn_self', 'negative_yn','text_x','emoticon','Positive','Negative','senti_score','distortion_yn', 'magnitude']
...
###processed by LIWC
data_dep['text_x'].to_csv('liwc.csv')
from sklearn import preprocessing
text_liwc = pd.read_csv('distortion_liwc_1000.csv')
liwc = text_liwc.loc[:,'function':'OtherP'].values
liwc_scaled = preprocessing.scale(liwc)


#####assign a row number to the last column for case checking later
...
####combine with liwc
x = np.concatenate((x, liwc_scaled), axis=1)


#####assign a row number to the last column for case checking later
row_num = np.arange(0,912).reshape(912, 1)
x = np.concatenate((x, row_num), axis=1)
...
x_train1, x_test1, y_train, y_test = train_test_split(x, y, test_size = 0.30, random_state = 0)

#delete the row num column 
x_train = np.delete(x_train1, 5522, 1) 
x_test = np.delete(x_test1, 5522, 1) 

#####later we can combine the row number with the prediction result
x_test_rowid = x_test1[:, [5522]]
...
####SVM 
svc_clf = SVC(C=50, cache_size=200, class_weight='balanced', coef0=0.0,
...
####remove the row num in train data
x2 = np.delete(x, 5522, 1) 
accuracies = cross_val_score(estimator = svc_clf, X = x2, y = y, cv = 10)
print(accuracies)
...
#[ 0.73913043  0.70652174  0.72826087  0.56521739  0.65217391  0.73626374
#  0.68131868  0.7         0.74444444  0.73333333]
####concatecate results and analysis the wrong cases

y_test_t = y_test.reshape(272,1)
y_pred_t = y_pred.reshape(272,1)
x_test_rowid_t = x_test_rowid.reshape(272,1)

result = np.concatenate((y_test_t, y_pred_t), axis=1)
result = np.concatenate((x_test_rowid_t, result), axis=1)

l3=[] #l[0] BOOL True if there is an emoji and l[0][0] is going to be the value
for i in result:
    if i[1] == i[2]:
        l3.append((True))
    else:
        l3.append((False, i[0], i[1], i[2]))

l3  ####now you can check which prediction is wrong on the text table

#data_dep['text_x'].to_csv('view_text.csv')
#pd.DataFrame(l).to_csv('result_list.csv')
...
# In[70]:
####oversample after crossed validation so that no duplicated cases in train set and test set
...
xy_df.iloc[:,-1]
df_minority = xy_df[xy_df.iloc[:,-1]==1]   ##218
df_majority = xy_df[xy_df.iloc[:,-1]==2]   #420
...
from sklearn.svm import SVC
clf = SVC(C=50, class_weight=None, coef0=0.0, degree=3, gamma='auto', kernel='rbf',
    max_iter=-1, probability=False, random_state=None, shrinking=True,
    tol=0.001, verbose=False)
...
y_test_t = y_test.reshape(284,1)
y_pred_t = y_pred.reshape(284,1)
x_test_rowid_t = x_test_rowid.reshape(284,1)

result = np.concatenate((y_test_t, y_pred_t), axis=1)
result = np.concatenate((x_test_rowid_t, result), axis=1)  ####now you can check which prediction is wrong on a table
